...
export function initLiquidacion(container) {
  container.innerHTML = `
    <div class="tool-card">
      ...
      <div class="form-row">
        ...
        <div class="field-group">
          <label for="liq-empleador">Empleador / Demandada</label>
          <input type="text" id="liq-empleador" placeholder="Ej: Empresa SA" autocomplete="off">
        </div>
      </div>
  ...
  if (!document.getElementById('liq-extra-styles')) {
    ...
    st.textContent = `
      ...
      .liq-tabla th { text-align:left; padding:0.5rem 0.7rem; background:var(--color-card,#1e2130); color:var(--color-accent,#c9a84c); font-size:0.82rem; text-transform:uppercase; letter-spacing:.04em; border-bottom:2px solid var(--color-accent,#c9a84c); }
      ...
      .liq-total-row td { font-weight:700; background:rgba(201,168,76,.10); color:var(--color-accent,#c9a84c); font-size:1.05rem; }
      .liq-total-grande { font-size:1.6rem; font-weight:800; color:var(--color-accent,#c9a84c); margin-top:1rem; text-align:right; }
  ...
  // Objetos de la acción según causa del egreso
  const causaObjetos = {
  ...
  function calcular() {
    ...
    const empleador = container.querySelector('#liq-empleador').value.trim();
    ...
    const autoCaratula = nombre && empleador
      ? `${nombre} c/ ${empleador} s/ ${causaObjetos[causa]}`
      : (nombre ? `${nombre} s/ ${causaObjetos[causa]}` : '');
    ...
    // — Exportar PDF
    container.querySelector('#liq-pdf').addEventListener('click', () => {
      const filasHtml = conceptos.map(c => `
        <tr${c.esDescuento ? ' style="color:#c00"' : ''}>
          <td>${c.label}</td>
    ...
    // — Exportar CSV
    container.querySelector('#liq-csv').addEventListener('click', () => {
      const csvFilas = [
        ['Concepto', 'Monto ($)', 'Base de cálculo'],
        ...conceptos.map(c => [c.label, c.monto.toFixed(2), c.base]),
        ['TOTAL LIQUIDACIÓN', total.toFixed(2), ''],
        ['', '', ''],
        ['Trabajador', nombre, ''],
        ['Empleador', empleador, ''],
        ['Carátula', autoCaratula, ''],
        ['Causa', causaTextos[causa], ''],
        ['Período', `${ingreso.toLocaleDateString('es-AR')} → ${egreso.toLocaleDateString('es-AR')}`, ''],
        ['Antigüedad', antiguedadTexto, ''],
      ];
      exportarCSV('Liquidacion_LCT' + (nombre ? '_' + nombre : ''), csvFilas);
    });
  }
}
